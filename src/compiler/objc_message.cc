...
namespace google { namespace protobuf { namespace compiler { namespace objectivec {
  ...
  void MessageGenerator::GenerateSource(io::Printer* printer) {
    ...
    printer->Print(
      ...
      "  NSData* data = [decoder decodeObjectForKey:@\"pbdata\"];\n"
      "  if ( !(self = (id)[[self class] parseFromData:data]) ) return nil;\n"
      "  return self;\n"
      "}\n");
    
    printer->Print(
      "- (void) encodeWithCoder:(NSCoder *)encoder {\n"
      "  [encoder encodeObject:self.data forKey:@\"pbdata\"];\n"
      "}\n");
      
      printer->Print(
        "- (id) copyWithZone:(NSZone *)zone {\n"
        "  id copy;\n"
        "  $classname$_Builder * builder = [self builder];\n"
        "  if ( !(copy = (id)[builder build]) ) return nil;\n"
        "  return copy;\n"
        "}\n",
        "classname", ClassName(descriptor_));
